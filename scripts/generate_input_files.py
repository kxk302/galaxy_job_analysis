...
def generate_input_files(jobs_file, datasets_file, numeric_metrics_file, output_dir):
  ...
  for index, row in tools_df.iterrows():
    ...
    for index_in, row_in in tool_jobs_df.iterrows():
      a_job_id = row_in['id']
      if index_in % 100 == 0:
        print(f'index_in: {index_in}, job_id: {a_job_id}, , tool_id: {a_tool_id}, tool_version: {a_tool_version}')
        print('\n')
    ...
    a_tool_all_jobs_df.to_csv(output_dir + '/' + os.path.basename(a_tool_id) + "_" + a_tool_version + ".csv", sep=",", index=False)
...
if __name__ == '__main__':
  ...
  generate_input_files(args.jobs_file, args.datasets_file, args.numeric_metrics_file, args.output_dir)